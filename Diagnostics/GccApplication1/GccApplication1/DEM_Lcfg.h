...
typedef struct {
	...
	Dem_CallbackReadDataFncType				DidReadFnc;
	uint8									PidOrDidSize;				// (1)
	boolean									PidOrDidUsePort;// (0..1)
} Dem_DidType; /** @req DEM136 */
...
typedef struct {
	...
	const Dem_DidType 	**FFIdClassRef; 	// (1..255)/** @req DEM039 */
...
typedef struct {
	...
	const Dem_PreDebounceAlgorithmClassType	*PreDebounceAlgorithmClass;	
...
// Event IDs
#define DEM_EVENT_ID_ENGINE_OVERHEAT   0x01
#define DEM_EVENT_ID_LOW_OIL_PRESSURE  0x02
#define DEM_EVENT_ID_BRAKE_FAILURE     0x03

extern const Dem_EventParameterType DemEventParameters[];
extern const Dem_ConfigSetType DemConfigSet;
