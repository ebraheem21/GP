...
//#define WIFI_SSID             "zaghloul"
//#define WIFI_PASSWORD         "25ea36a4"

/* 1. Define the WiFi credentials */
#define WIFI_SSID             "Amr"
#define WIFI_PASSWORD         "123456789"

/* 2. Define the API Key */
#define API_KEY               "AIzaSyCqOBAQCjC2wK7Hkhb2MfG7S28l9aDc8Yo"

/* 3. Define the user Email and password that alreadey registerd or added in your project */
#define USER_EMAIL            "amressam@fota.com"
#define USER_PASSWORD         "123456789"

/* 4. Define the Firebase storage bucket ID e.g bucket-name.appspot.com */
#define STORAGE_BUCKET_ID     "fotafota-7751f.appspot.com"

#define NOT_ACKNOWLEDGE       0xAB
#define JUMP_SUCCEDDED        0x01
#define ERASE_SUCCEDDED       0x03
#define WRITE_SUCCEDDED       0x01


#define FAULT_MESSAGE "Fault1"
#define FAULT_MESSAGE_LENGTH 6
...
void loop()
{
    ...
    /*uint8_t packetLength = FAULT_MESSAGE_LENGTH;
    
    if (ReceiveReplayFromSTM(packetLength))
    {
        Serial.println("Received 'Fault1' successfully.");
        Serial.print("Sending packet ... ");

    CAN.beginPacket(0x125);
    CAN.write(0x59);
    CAN.write(0x02);
    CAN.write(0x7F);
    CAN.write(0x0A);
    ...
    {
        Serial.println("Failed to receive 'Fault1'.");
    }*/
...
void handleCANMessage() {
    ...
    if (CAN.packetRtr()) {
        ...
        // Read the command from the CAN bus
        byte recCommand = CAN.read();
        Serial.println(recCommand, HEX);

        switch(recCommand) {
            case 0x55:  Send_CMD_Read_CID();               break;
            case 0x44:  Send_CMD_Jump_To_Application();    break;
            case 0x33:  Send_CMD_Jump_to_Bootloader();     break;
            case 0x22:  Send_CMD_Erase_Flash();            break;
            case 0x11:  Send_CMD_Upload_Application();     break;
        }
...
void SendCMDPacketToBootloader(uint8_t packetLength)
{
    for(uint8_t dataIndex = 0 ; dataIndex < packetLength ; dataIndex++){
        mySerial.write(packet[dataIndex]);
...
void ReceiveReplayFromBootloader(uint8_t packetLength)
{
    for(uint8_t dataIndex = 0 ; dataIndex < packetLength ; dataIndex++)
    {
        ...
        packet[dataIndex] = mySerial.read();
...
void Send_CMD_Read_CID()
{
    ...
    *((uint32_t*)((uint8_t*)packet + 2)) = calculateCRC32((uint8_t*)packet, 2);
    ...
    client.publish("/FOTA/Reply", Replay, false); 
}
...
void Send_CMD_Jump_To_Application()
{
    ...
    *((uint32_t*)((uint8_t*)packet + 2)) = calculateCRC32((uint8_t*)packet, 2);
    ...
    if(JUMP_SUCCEDDED == packet[0]){ sReplay += "Jump Success"; Serial.println("Jump Success");  }
    else{ sReplay += "Jump Fail"; Serial.println("Jump Unsuccess"); }

    sReplay.toCharArray(Replay, sReplay.length()+1);
    client.publish("/FOTA/BootReply", Replay, false); 
}

void Send_CMD_Jump_to_Bootloader()
{
char Replay[16] = {0};
    String sReplay = "";
    packet[0] = 5;
    ...
    /**((uint32_t*)((uint8_t*)packet + 2)) = calculateCRC32((uint8_t*)packet, 2);
    SendCMDPacketToBootloader(6);*/
    char Update_Request[] = "UPDATE_REQUEST";
    uint8_t UPDATE_REQUEST_LENGTH = 14;
    for(uint8_t dataIndex = 0 ; dataIndex < UPDATE_REQUEST_LENGTH ; dataIndex++){
        mySerial.write(Update_Request[dataIndex]);
        Serial.print(Update_Request[dataIndex]);
        Serial.print(" ");
    }
    Serial.println("\n");
    ...
    if(JUMP_SUCCEDDED == packet[0]){ sReplay += "Jump Success"; }
...
void Send_CMD_Erase_Flash()
{
    ...
    *((uint32_t*)((uint8_t*)packet + 2)) = calculateCRC32((uint8_t*)packet, 2);
    ...
    if(ERASE_SUCCEDDED == packet[0]){ sReplay += "Erase Success"; Serial.println("Erase Success"); EraseFlag = 0;  }
    else{ sReplay += "Erase Unsuccess"; Serial.println("Erase Unsuccess"); EraseFlag = 1; }
...
void Send_CMD_Upload_Application()
{
    ...
    if (Firebase.ready())
    {
      if(EraseFlag == 1)
      {
        ...
        if (!Firebase.Storage.download(&fbdo, STORAGE_BUCKET_ID /* Firebase Storage bucket id */, "App.bin" /* path of remote file stored in the bucket */, "/update.bin" /* path to local file */, mem_storage_type_flash /* memory storage type, mem_storage_type_flash and mem_storage_type_sd */, fcsDownloadCallback /* callback function */))
        ...
        while(file.available())
        {
            ...
            while(CounterByte < 64)
            {
                ...
                char HexData = strtol(buffer,NULL,16);
            ...
            *((uint32_t*)((uint8_t*)packet + 71)) = calculateCRC32((uint8_t*)packet, 71);
            ...
            while(CounterByte < 64)
            {
              mySerial.write(packet[7+CounterByte]);
              CounterByte++;
            }
            ...
            else{ isFailed = 1; sReplay += "Write Failed"; break; }
        ...
        if(isFailed){ client.publish("/FOTA/Reply", "Upload Failed", false); Serial.println("Upload Unsuccess"); }
        else{ client.publish("/FOTA/Reply", "Upload Success", false);
